...
- Central Shahir VI guide stage that remains visually present on desktop and mobile.
- Reuses the project’s approved Shahir VI poster artwork (`awwab-poster.svg`) and the documented `awwab-idle.mp4` integration slot.
...
- Speech/TTS adapter using the browser voice layer; idle/talking state hooks are ready for real Shahir VI media.
...
When approved Shahir VI idle/talking video or WAV assets are located, place them under `assets/video/` or `assets/audio/` and update the existing media adapter in `app.js`; the approved poster remains the lightweight fallback. Do not preload large video files in the initial package.
